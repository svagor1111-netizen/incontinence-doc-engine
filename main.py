...
def synced_order(payload: IncontinenceRequest, items: List[str]) -> IncontinenceOrder:
    ...
    # SIZE FIX ONLY:
    # if user did not specify any size -> all stay blank
    # if user specified one or more -> keep only what came in
    size_specified = any([
        order.size_s,
        order.size_m,
        order.size_l,
    ...
    if not size_specified:
        ...
        order.size_s = bool(order.size_s)
        order.size_m = bool(order.size_m)
        order.size_l = bool(order.size_l)
        order.size_xl_xxl = bool(order.size_xl_xxl)
...
def create_dme_documents(payload: IncontinenceRequest, request: Request):
    ...
    # IMPORTANT:
    # build URLs from the actual current request host to avoid broken bucket/base-url links
